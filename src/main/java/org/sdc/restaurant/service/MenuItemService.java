...
public class MenuItemService {
    ...
    public List<MenuItem> search(String keywords, SearchMenuType searchMenuType) {
        return switch (searchMenuType) {
            case ALL ->
                    menuItems.stream().filter(item -> Helper.containKeyword(item.getContent(), keywords)).collect(Collectors.toList());
            case NAME ->
                    menuItems.stream().filter(item -> Helper.containKeyword(item.getName(), keywords)).collect(Collectors.toList());
            case DESCRIPTION ->
                    menuItems.stream().filter(item -> Helper.containKeyword(item.getDescription(), keywords)).collect(Collectors.toList());
            case TYPES ->
                    menuItems.stream().filter(item -> Helper.containKeyword(item.getTypes(), keywords)).collect(Collectors.toList());
        };
    }
