package com.sdc.restaurantmanagement.constant;

public class Constant {
    public static final String NOT_FOUND_THE_MENU_ITEM_WITH_ID = "Can't find the menu item with id ";
    public static final String NOT_FOUND_THE_BILL_ORDER_WITH_ID = "Can't find the bill order with id ";
    public static final String NO_BILL_ORDER_WITH_ID = "There is no bill order with id ";

    public static final String INVALID_URL_EXCEPTION = "Image URL is invalid";
}
